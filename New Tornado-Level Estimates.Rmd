...
OH2010.sf <- st_transform(OH2010.sf, crs = st_crs(TornB.sf))
```
...
TornSCOH.sf <- TornSC.sf %>%
  filter(st == "OH")

((sum(TornSCOH.sf$TotlPpl) - sum(out$B01001_001E[-c(2,31,32)]))/sum(TornSCOH.sf$TotlPpl)) * 100

cor.test(TornSCOH.sf$TotlPpl,out$B01001_001E[-c(2,31,32)])
```

Subset all 2016 tornadoes.
```{r}
...
Get Census tracts for all contiguous U.S. states
```{r}
us = unique(fips_codes$state)[c(1, 3:8, 10:11, 13:51)]

us2016.sf = reduce(map(us, function(x) {
  get_acs(state = x,
          geography = "tract", 
          variables = ("B01001_001E"),
          year = 2012,
...
us2016.sf <- st_transform(us2016.sf, crs = st_crs(Torn2016.sf))
```
...
```{r}
#TornSC2016.sf <- TornSC.sf %>%
#  filter(Year == 2016)

# Percent error
((sum(TornSC.sf$TotlPpl) - sum(out$B01001_001E[-c(189, 293, 465, 474, 1169, 2005, 2187, 2188, 2205, 2206, 2208)]))/sum(TornSC.sf$TotlPpl)) * 100

#Pearson Correlation
cor.test(TornSC.sf$TotlPpl, out$B01001_001E[-c(189, 293, 465, 474, 1169, 2005, 2187, 2188, 2205, 2206, 2208)])

# RMSE
Diffpop = sum(TornSC2016.sf$TotlPpl) - sum(out$B01001_001E)
RMSEpop = sqrt((abs(Diffpop)^2)/58)
```
