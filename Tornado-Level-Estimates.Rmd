...
**This code has been run and is available starting on line 735**
...
Male Population (Under 18) (P008003-P008020/B01001_003E-B01001_006E)
...
Female Population (Under 18) (P008042-P008059/B01001_027E-B01001_030E)
...
US2010.sf = US2010.sf %>%
  mutate(TotalPop_2010 = TotalPop,
         ...
         Male18to44_2010 = Male18.19 + Male20 + Male21 + Male22.24 + Male25.29 +
         Male30.34 + Male35.39 + Male40.44,
         ...
         Female18to44_2010 = Female18.19 + Female20 + Female21 + Female22.24 
         + Female25.29
...
Find margins of error for the aggregated count data (in percentage error)
```{r}
df = data.frame(US2010.sf) %>%
  mutate(TotalPopM = TotalPopM/(TotalPop + TotalPopM),
         TotalMaleM = TotalMaleP/(TotalMale + TotalMaleP),
         MaleUnder18M = sqrt(MaleUnder5M^2 + Male5.9M^2 + Male10.14M^2 +
         Male15.17M^2)/((MaleUnder5 + Male5.9 + Male10.14 + Male15.17) +
         (sqrt(MaleUnder5M^2 + Male5.9M^2 + Male10.14M^2 + Male15.17M^2))),
         Male18to44M = sqrt(Male18.19M + Male20M + Male21M + Male22.24M + Male25.29M +
         Male30.34M + Male35.39M + Male40.44M)/((Male18.19 + Male20 + Male21 + 
         Male22.24 + Male25.29 + Male30.34 + Male35.39 + Male40.44) + (Male18.19M +
         Male20M + Male21M + Male22.24M + Male25.29M + Male30.34M + Male35.39M +
         Male40.44M)),
         Male45to64M = sqrt(Male45.49M + Male50.54M + Male55.59M + Male60.61M +
         Male62.64M)/((Male45.49 + Male50.54 + Male55.59 + Male60.61 + Male62.64) +
         sqrt(Male45.49M + Male50.54M + Male55.59M + Male60.61M + Male62.64M)),
         MaleOver65M = sqrt(Male65.66M + Male67.69M + Male70.74M + Male75.79M 
         + Male80.84M
         + MaleOver85M)/((Male65.66 + Male67.69 + Male70.74 + Male75.79 + Male80.84
         + MaleOver85) + sqrt(Male65.66M + Male67.69M + Male70.74M + Male75.79M +
         Male80.84M + MaleOver85M)),
         TotalFemaleM = TotalFemaleM/(TotalFemale + TotalFemaleM),
         FemaleUnder18M = sqrt(FemaleUnder5M + Female5.9M + Female10.14M +
         Female15.17M)/((FemaleUnder5 + Female5.9 + Female10.14 + Female15.17) +
         sqrt(FemaleUnder5M + Female5.9M + Female10.14M + Female15.17M)),
         Female18to44M = sqrt(Female18.19M + Female20M + Female21M + Female22.24M +
         Female25.29M + Female30.34M + Female35.39M + Female40.44M)/((Female18.19 +
         Female20 + Female21 + Female22.24 + Female25.29 + Female30.34 + Female35.39 +
         Female40.44) + sqrt(Female18.19M + Female20M + Female21M + Female22.24M +
         Female25.29M + Female30.34M + Female35.39M + Female40.44M)),
         Female45to64M = sqrt(Female45.49M + Female50.54M + Female55.59M + 
         Female60.61M + Female62.64M)/((Female45.49 + Female50.54 + Female55.59 +
         Female60.61 + Female62.64) + sqrt(Female45.49M + Female50.54M + 
         Female55.59M + Female60.61M + Female62.64M)),
         FemaleOver65M = sqrt(Female65.66M + Female67.69M + Female70.74M + Female75.79M 
         + Female80.84.1 + FemaleOver85.1)/((Female65.66 + Female67.69 + Female70.74 +
         Female75.79 + Female80.84 + FemaleOver85) + sqrt(Female65.66M + Female67.69M +
         Female70.74M + Female75.79M + Female80.84.1 + FemaleOver85.1)),
         WhiteM = WhiteM/(White + WhiteM),
         BlackM = BlackM/(Black + BlackM),
         TotalHousingUnitsM = TotalHousingUnitsM/(TotalHousingUnits + TotalHousingUnitsM),
         MobileHomesM = MobileHomesM/(MobileHomes + MobileHomesM),
         MedianIncomeM = MedianIncomeM/(MedianIncome + MedianIncomeM)) %>%
  dplyr::select(STATE, COUNTY, NAME_E, TotalPopM, TotalMaleM, MaleUnder18M, Male18to44M,
         Male45to64M, MaleOver65M, TotalFemaleM, FemaleUnder18M, Female18to44M,
         Female45to64M, FemaleOver65M, WhiteM, BlackM, TotalHousingUnitsM, MobileHomesM, MedianIncomeM)
```

Summarize the MOE from 2010 ACS data.
```{r}
summary(df)
```

Margins of error for population density data are more complicated. Manually edit here:
```{r}
...
Investigate the mobile homes MOE in more detail.
```{r}
df2 = data.frame(US2010.sf) %>%
  mutate(MobileHomesM = MobileHomesM/(MobileHomes + MobileHomesM)) %>%
  dplyr::select(MobileHomes, MobileHomesM)

df2 = df2[df2$MobileHomes != 0,]
...
us_geo <- read_shape("states/cb_2015_us_state_20m.shp", as.sf = TRUE, stringsAsFactors = FALSE)
...
Our results are validated by comparing 20 actual and modeled paths and their associated estimates. 

**To see this validation download Demographic-Comparisons.Rmd**
...
A = tm_shape(us_geo, projection ="+init=epsg:2163") +
  tm_borders() +
  tm_shape(sfdf3) +
  tm_bubbles("WhiteCas", col = "red", 
             alpha = 0.3, 
             sizes.legend = c(200, 400, 800, 1200), 
             title.size="Estimated White Casualties", 
             scale = 5/3) +
  tm_shape(us_geo) + 
  tm_borders(col = "grey", alpha = 0.3) +
...
B = tm_shape(us_geo, projection ="+init=epsg:2163") +
  ...
  tm_bubbles("BlackCas", 
             ...
             sizes.legend = c(200, 400, 800, 1200), 
             alpha = 0.3, 
             title.size="Estimated Black Casualties", 
             scale = 4/3) +
  tm_shape(us_geo) + 
  tm_borders(col = "grey", alpha = 0.3) +
...
tmap_arrange(A, B, ncol = 1) #Export as 8.5x6 PDF
```
...
Using the ratio of the young population (under 18) relative to total population, the number of young casualties per tornado can be estimated. Similarly, using the ratio of the older adult population (over 65) relative to total population, the number of older adult casualties per tornado can be estimated.
```{r}
sfdf = TornSC.sf %>%
...
A = tm_shape(us_geo, projection ="+init=epsg:2163") +
  tm_borders() +
  tm_shape(sfdf2) +
  tm_bubbles("YoungCas", col = "red", 
             alpha = 0.3, 
             sizes.legend = c(100, 200, 300, 400), 
             title.size="Estimated Young Casualties", 
             scale = 5/3) +
  tm_shape(us_geo) + 
  tm_borders(col = "grey", alpha = 0.3) +
  tm_style('white', title="A") +
...
B = tm_shape(us_geo, projection ="+init=epsg:2163") +
  tm_borders() +
  tm_shape(sfdf3) +
  tm_bubbles("OldCas", col = "blue", 
             sizes.legend = c(100, 200, 300, 400), 
             alpha = 0.3, 
             title.size="Estimated Older Adult Casualties", 
             scale = 4/3) +
  tm_shape(us_geo) + 
  tm_borders(col = "grey", alpha = 0.3) +
  tm_style('white', title="B") +
...
The median number of young casualties for the set of 2,201 tornadoes is 0.64 people with an interquartile range of 0.28 and 1.78. In comparison, the median number of older adult casualties is 0.37 with an interquartile range of 0.17 and 0.99. The average number of young casualties is 2.91 and the average number of older adult casualties is 1.64. Of the top 10 tornadoes ranked by young and older adult casualties, only one (1998 Spencer, South Dakota tornado) occurred in a state outside the southern Great Plains or Southeast.
