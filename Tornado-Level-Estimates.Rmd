...
df = data.frame(US2010.sf) %>%
  ...
  select(STATE, COUNTY, NAME_E, TotalPopM, TotalMaleM, MaleUnder18M, Male18to44M,
...
```{r}
x = (US2010.sf$TotalPop/US2010.sf$Shape_Area) * 10^6
xx = (US2010.sf$TotalPopM/US2010.sf$Shape_Area) * 10^6
xxx = xx/(x + xx)
summary(xxx)
```
...
df2 = data.frame(US2010.sf) %>%
  ...
  select(MobileHomes, MobileHomesM)
...
Using the ratio of the young population (under 18) relative to total population, the number of young casualties per tornado can be estimated. Similarly, using the ratio of the elderly population (over 65) relative to total population, the number of elderly casualties per tornado can be estimated.
```{r}
sfdf = TornSC.sf %>%
...
B = tm_shape(us_geo, projection ="+init=epsg:2163") +
  ...
  tm_bubbles("OldCas", col = "blue", 
             sizes.legend = c(100, 200, 300, 400), 
             alpha = 0.3, 
             title.size="Estimated Elderly Casualties", 
             scale = 4/3) +
  tm_shape(us_geo) + 
  tm_borders(col = "grey", alpha = 0.3) +
  tm_style('white', title="B") +
